...
int get_player_move(int player, BOARD arr) //유저 선택 함수 선언
{
	POINT input; //구조체 변수 선언

	while (1) //무한루프
	{
		printf("(x, y)좌표를 입력(종료: -1, -1): ");
		scanf("%d %d", &input.x, &input.y);

		if (input.x - 1 == -1 && input.y - 1 == -1) //종료 조건이면
			return 1; //무한루프를 멈추고 비정상 종료

		if ((input.x < 1 || input.x > ROW) || (input.y < 1 || input.y > COLUMN)) //인덱스 범위를 벗어나서 입력하면
		{
